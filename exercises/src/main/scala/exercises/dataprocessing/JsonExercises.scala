...
object JsonExercises {
  ...
  def trimAll(json: Json): Json =
    json match {
      case _: JsonNumber | _: JsonBoolean | JsonNull => json
  ...
  def anonymize(json: Json): Json =
    json match {
      ...
      case JsonObject(obj)           => JsonObject(obj.map { case (k, v) => (k, anonymize(v)) })
  ...
  def search(json: Json, substring: String, depth: Int): Boolean =
    if (depth < 0) false
    else
      json match {
        case JsonNumber(_) | JsonBoolean(_) | JsonNull => false
        case JsonString(text)                          => text.contains(substring)
        case JsonObject(obj) =>
          obj.values.exists(search(_, substring, depth - 1))
        case JsonArray(arr) => arr.exists(search(_, substring, depth - 1))
      }
  ...
  def depth(json: Json): Int =
    json match {
      ...
      case JsonArray(arr) =>
        arr.map(depth).maxOption.fold(0)(_ + 1)
    }
