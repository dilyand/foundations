...
object TemperatureExercises {
  ...
  def foldLeft[From, To](
    parList: ParList[From],
    default: To
  )(combineE: (To, From) => To, combineI: (To, To) => To): To = {
    def foldPartition(partition: List[From], default: To)(combine: (To, From) => To): To =
      partition.foldLeft(default)(combine)

    val intermediate = parList.partitions.map(foldPartition(_, default)(combineE))
    intermediate.foldLeft(default)(combineI)
  }
